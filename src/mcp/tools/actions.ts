...
export const actionsListTool: McpTool = {
  ...
  run(context) {
    ...
    if (!deviceIds.length || !allowed.size) return complete([]);
    ...
    if (!workspaceRows.length) return complete([]);
    ...
    return complete(rows.filter(action => allowed.get(action.id) === actionHash(action.command, action.cwd)).map(action => ({
...
export const actionRunTool: McpTool = {
  ...
  async run(context, args, params) {
    ...
    if (!action || !expectedHash || expectedHash !== actionHash(action.command, action.cwd)) return complete("Action is not in this MCP grant or changed after authorization.", true);
    ...
    if (!deviceIds.length || valid.length !== deviceIds.length || deviceIds.some(id => !allowedDevices.has(id))) return complete("One or more machines are outside this Action's workspace or MCP grant.", true);
    if (action.confirm) {
    ...
    return complete({ actionId, action: action.name, results });
  },
};

...
pub(super) fn check(runtime: ComponentExecutionRuntime) -> anyhow::Result<()> {
    let executable = std::env::current_exe()?
        ...
        .replace('\'', "'\\''");
    let script = format!(
        "'{executable}' text-fixture first && '{executable}' text-fixture second >&2 ; exit 7"
    );
    let execution = runtime
        .start(ProcessStartRequest {
            execution_id: probe_id("mcp-stderr-read"),
    ...
    anyhow::ensure!(
        read.chunks
            .iter()
            .any(|chunk| matches!(chunk.stream, rc_node::StreamKind::Stderr)
                && chunk.bytes == b"second"),
        "redirected stderr was lost"
    );
    for _ in 0..3 {

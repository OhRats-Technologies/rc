...
fn vanished_path_is_retried_without_an_invalid_status() -> anyhow::Result<()> {
    ...
    let path = components.join("provider.wasm");
    fs::create_dir_all(&components)?;
    fs::write(&path, b"present during the desired scan")?;
    let mut runtime = Runtime::new(components)?;
...
fn publication_fence_defers_reconciliation() -> anyhow::Result<()> {
    let directory = tempfile::tempdir()?;
    let components = directory.path().join("components");
    let mut runtime = Runtime::new(components.clone())?;
    fs::write(components.join(UPDATE_FENCE), b"transaction\n")?;
    fs::write(components.join("partial.wasm"), b"not complete")?;

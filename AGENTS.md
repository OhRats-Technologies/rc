...
- RC Lock is enforced locally by the Node. Its persisted authority snapshot owns member roles, passkey credential public keys, and API signing public keys/scopes. First initialization is TOFU from the currently trusted instance and may never overwrite an existing lock. Every later snapshot must be signed by an existing Owner using a passkey-authorized control key; server database state alone is not authority. Accepting a new snapshot invalidates every live encrypted control session so removed/demoted identities must reopen against the new Node-local authority.
- Human browser/CLI control grants default to 30 days, but each live control connection uses fresh X25519 client and Node ephemeral keys. Mix those with the Node's pinned static X25519 key via HKDF-SHA256 and encrypt control frames with AES-256-GCM. The Node's Ed25519 identity signs the live handshake. Do not send process commands, terminal data, signals, node updates, or node removal outside this encrypted Node-verified channel.

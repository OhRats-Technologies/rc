...
- Global navigation uses the persistent sidebar. Workspace folders disclose device children; the folder click itself does not navigate. Keep child lists bounded and context actions in the workspace ellipsis menu. Route-specific actions belong on their pages, not in global chrome or dialogs.

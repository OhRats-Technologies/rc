...
- Global navigation uses the persistent sidebar. Workspace folders disclose device children; the folder click itself does not navigate. Disclosure height animates rather than snapping. Keep child lists bounded and context actions in the workspace ellipsis menu. Treat the folder + ellipsis as one visual focus group, dismiss menus on outside interaction/Escape, and keep simple workspace edits such as rename inline in that menu instead of duplicating them as separate pages. Route-specific actions belong on their pages, not in global chrome or dialogs.

...
func commandDefaults(stateDir, urlFlag string) (string, string, nodeConfig) {
	...
	server := urlFlag
	if server == "" { server = env("RELAY_URL", config.Server) }
...
func enrollCommand(args []string) error {
	...
	if flags.NArg() != 1 { return errors.New("usage: ohrats-relay enroll [flags] TOKEN") }
	...
	name := *nameFlag
	if name == "" { name = env("RELAY_NAME", config.Name) }
	if *serverFlag != "" { config.Server = server }
	if *nameFlag != "" { config.Name = name }
	if *serverFlag != "" || *nameFlag != "" {
		if err := saveConfig(dir, config); err != nil { return err }
	}
	value, err := enroll(server, flags.Arg(0), name)
	if err != nil { return err }
	if err := saveState(dir, value); err != nil { return err }
...
func runNode(args []string) error {
	...
	if errors.Is(err, os.ErrNotExist) { return errors.New("not enrolled; run ohrats-relay enroll TOKEN") }
	if err != nil { return err }
...
func uninstallCommand(args []string) error {
	...
	if dir == defaultStateDir() { _ = os.RemoveAll(legacyStateDir()) }
	executable, _ := os.Executable()
	if filepath.Base(executable) == "ohrats-relay" { _ = os.Remove(executable) }
	if executable != "" { _ = os.Remove(filepath.Join(filepath.Dir(executable), "relay-agent")) }

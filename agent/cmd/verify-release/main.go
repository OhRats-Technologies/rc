...
func main() {
	...
	if err != nil {
		fatal("read signature: %v", err)
	}
	manifest, err := releaseverify.Verify(manifestBytes, signatureBytes)
	if err != nil {
		fatal("verify manifest: %v", err)
	}
	if manifest.Version != os.Args[4] {
		fatal("manifest version %s does not match %s", manifest.Version, os.Args[4])
	}
	for _, artifact := range manifest.Artifacts {
		hash, err := releaseverify.FileSHA256(filepath.Join(os.Args[3], artifact.Name))
		if err != nil {
			fatal("hash %s: %v", artifact.Name, err)
		}
		if hash != artifact.SHA256 {
			fatal("hash mismatch for %s", artifact.Name)
		}
	}

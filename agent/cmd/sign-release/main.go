...
func main() {
	...
	for _, target := range [][2]string{{"darwin", "amd64"}, {"darwin", "arm64"}, {"linux", "amd64"}, {"linux", "arm64"}} {
		...
		if err != nil { fatal("hash %s: %v", name, err) }
	...
	if err != nil { fatal("manifest: %v", err) }
	data = append(data, '\n')
	signature := ed25519.Sign(privateKey, data)
	if err := os.WriteFile(manifestPath, data, 0644); err != nil { fatal("write manifest: %v", err) }
	encoded := append([]byte(base64.RawURLEncoding.EncodeToString(signature)), '\n')
	if err := os.WriteFile(signaturePath, encoded, 0644); err != nil { fatal("write signature: %v", err) }
}

func loadKey(path string) ed25519.PrivateKey {
	data, err := os.ReadFile(path)
	if err != nil { fatal("read private key: %v", err) }
	block, _ := pem.Decode(data)
	if block == nil { fatal("invalid private key PEM") }
	parsed, err := x509.ParsePKCS8PrivateKey(block.Bytes)
	if err != nil { fatal("parse private key: %v", err) }
	key, ok := parsed.(ed25519.PrivateKey)
	if !ok { fatal("private key is not Ed25519") }
	return key
}

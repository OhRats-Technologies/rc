...
for value in "$package_version" "$cli_version" "$server_version" "$changelog_version"; do
  if [ -z "$value" ]; then
...
if [ "$package_version" != "$cli_version" ] || [ "$cli_version" != "$server_version" ] || [ "$server_version" != "$changelog_version" ]; then
  echo "RC versions differ: package=$package_version cli=$cli_version server=$server_version changelog=$changelog_version" >&2
  exit 1
fi

...
function credentialJSON(credential: PublicKeyCredential) {
  ...
  if (native) return native.call(credential);
  ...
  if (response instanceof AuthenticatorAttestationResponse) {
    ...
    output.response = {
      clientDataJSON: bytesToB64url(response.clientDataJSON), authenticatorData: bytesToB64url(response.authenticatorData),
      signature: bytesToB64url(response.signature), ...(response.userHandle ? { userHandle: bytesToB64url(response.userHandle) } : {}),
    };
  }
  return output;
}
...
export async function createPasskey(path: string, verifyPath: string, data: Record<string, unknown>) {
  requirePasskeys();
  const start = await api<{ ceremonyId: string; options: any }>(path, { method: "POST", body: JSON.stringify(data) });
...
export async function authenticatePasskey() {
  requirePasskeys();
  const start = await api<{ ceremonyId: string; options: any }>("/api/v1/auth/login/options", { method: "POST", body: "{}" });
  ...
  return api("/api/v1/auth/login/verify", {
    method: "POST", body: JSON.stringify({ ceremonyId: start.ceremonyId, response: credentialJSON(credential) }),
  });
}

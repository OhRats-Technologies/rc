  ...
  return htmlDocument({ title, scripts: mode === "join" ? [] : ["auth"], body:
    <section className="auth-shell"><div className="ohrats-grid auth-grid" aria-hidden="true"/><div className="auth-content">
      ...
      {mode === "login" && <><p className="muted">Use a passkey or a full-access API token.</p>
        <form id="login-form" className="auth-form"><button className="primary-button" type="submit">SIGN IN WITH PASSKEY</button></form><div className="auth-divider"><span>OR</span></div>
      ...
      {mode === "join" && <><p className="muted">Join this workspace with your current account.</p><form method="post" action="/workspaces/join" className="auth-form"><input type="hidden" name="token" value={invite}/><button className="primary-button" type="submit">JOIN WORKSPACE</button></form></>}

  ...
  return htmlDocument({ title, scripts: mode === "join" || mode === "invalid-invite" ? [] : ["auth"], body:
    <section className="auth-shell"><div className="ohrats-grid auth-grid" aria-hidden="true"/><div className="auth-content" data-auth-next={options.next || ""}>
      ...
      {mode === "login" && <><p className="muted">Use your passkey to sign in.</p>
        <form id="login-form" className="auth-form"><button className="or-button" type="submit">SIGN IN WITH PASSKEY</button></form></>}
...
export function cliLoginPage(user: import("../../../src/core").User, code: string, approved = false, error = "") {
  return htmlDocument({ title: approved ? "CLI authorized" : "Authorize CLI", body:
    <section className="auth-shell"><div className="ohrats-grid auth-grid" aria-hidden="true"/><div className="auth-content">
      <p className="eyebrow">RELAY / COMMAND LINE</p><h1>{approved ? "CLI authorized" : "Authorize CLI"}</h1>
      {approved ? <p className="page-copy">Return to your terminal. This tab can be closed.</p> : <>
        <p className="page-copy">Allow the Relay command line to act as <strong>{user.name}</strong> on your workspaces.</p>

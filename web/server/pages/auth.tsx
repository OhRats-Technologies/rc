...
export function authPage(mode: AuthMode, options: {
  invite?: string; authorized?: boolean; error?: string; workspaceName?: string; role?: "member" | "viewer";
} = {}) {
  ...
  const title = mode === "setup" ? "Create Relay" : mode === "register" || mode === "join" ? joinTitle
  ...
  return htmlDocument({ title, scripts: mode === "join" || mode === "invalid-invite" ? [] : ["auth"], body:
    <section className="auth-shell"><div className="ohrats-grid auth-grid" aria-hidden="true"/><div className="auth-content">
      ...
      {mode === "register" && <><p className="muted">Create a passkey to join as {options.role || "member"}.</p><form id="register-form" className="auth-form"><label>Name<input name="name" autoComplete="name" required/></label><input name="invite" type="hidden" value={invite}/><button className="or-button" type="submit">CREATE PASSKEY</button></form><a className="text-action" href={`/?invite=${encodeURIComponent(invite)}&signin=1`}>SIGN IN INSTEAD</a></>}
      {mode === "join" && <><p className="muted">Join as {options.role || "member"} with your current account.</p><form method="post" action="/workspaces/join" className="auth-form"><input type="hidden" name="token" value={invite}/><button className="or-button" type="submit">JOIN WORKSPACE</button></form></>}

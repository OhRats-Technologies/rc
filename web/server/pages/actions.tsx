...
export function actionsPage(user: User, workspaces: WorkspaceView[], actions: ActionView[], sidebar: "open" | "closed") {
  ...
  return htmlDocument({ title: "Actions", user, workspaces, path: "/actions", sidebar, body:
    <div className="page"><header className="page-header"><div><p className="eyebrow">ACTIONS</p><h1>Actions</h1></div>{ownerWorkspace && <a className="or-button" href={`/actions/new?workspace=${ownerWorkspace.id}`}>NEW ACTION</a>}</header>
...
export function actionPage(user: User, workspaces: WorkspaceView[], action: ActionView, devices: DeviceView[], sidebar: "open" | "closed", results: ActionRunResult[] = []) {
  ...
  return htmlDocument({ title: action.name, user, workspaces, path: `/actions/${action.id}`, sidebar, body:
    <div className="page"><header className="page-header"><div><p className="eyebrow">{action.workspace_name.toUpperCase()} / ACTION</p><h1>{action.name}</h1>{action.description && <p className="page-copy">{action.description}</p>}</div>{action.role === "owner" && <a className="or-button" href={`/actions/${action.id}/edit`}>EDIT</a>}</header>
      ...
      {action.role === "owner" && <section className="content-section danger-section"><SectionBadge index="03">Action</SectionBadge><form method="post" action={`/actions/${action.id}/delete`}><button className="text-action danger-text" type="submit">DELETE ACTION</button></form></section>}
    </div> });
}

...
export function actionConfirmPage(user: User, workspaces: WorkspaceView[], action: ActionView, devices: DeviceView[], selected: string[], sidebar: "open" | "closed") {
  ...
  return htmlDocument({ title: `Run ${action.name}`, user, workspaces, path: `/actions/${action.id}`, sidebar, scripts: ["action-run"], body:
    <div className="page narrow-form-page" data-action-run={action.id} data-action-command={action.command} data-action-cwd={action.cwd || ""}><header className="page-header"><div><p className="eyebrow">{action.workspace_name.toUpperCase()} / ACTION</p><h1>Run {action.name}?</h1></div></header>

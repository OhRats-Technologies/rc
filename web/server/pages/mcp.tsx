...
export function mcpConnectionsPage(user: User, workspaces: WorkspaceView[], sidebar: "open" | "closed", endpoint: string,
  ...
  return htmlDocument({ title: "MCP", user, workspaces, path: "/integrations/mcp", sidebar, scripts: ["mcp-page"], body:
    <div className="page"><header className="page-header"><div><p className="eyebrow">RC / MODEL CONTEXT PROTOCOL</p><h1>MCP</h1><p className="page-copy">Connect an AI agent to <strong>RC</strong> with this URL: <code>{endpoint}</code></p><p className="meta">CONFIG IDENTIFIER · <code>rc</code></p></div></header>
      <div className="settings-list">{grants.length ? grants.map(({ record, payload }) => <div className="setting-row" key={record.id}><div>
        ...
        : <p className="empty-state">No AI agents are connected yet. Add RC to your MCP client to start.</p>}</div>

...
function updateAvailable(agent: string, relay: string) {
  const parse = (value: string) => value.match(/^(\d+)\.(\d+)\.(\d+)/)?.slice(1).map(Number);
  const current = parse(agent), target = parse(relay);
  if (!current || !target) return agent !== relay;
...
export function devicePage(user: User, workspaces: WorkspaceView[], device: DeviceView, processes: RemoteProcess[], relayVersion: string, sidebar: "open" | "closed") {
  ...
  const hasUpdate = supportsUpdate && updateAvailable(device.agent_version, relayVersion);
  ...
  return htmlDocument({ title: device.name, user, workspaces, path: `/devices/${device.id}`, sidebar, scripts: ["live", "device"], body:
    <div className="page" data-device-page={device.id} data-relay-version={relayVersion}>
      ...
      <section className="device-summary">
        <dl className="facts">
          ...
          <div><dt>RELAY</dt><dd>{relayVersion}</dd></div>
        </dl>
        {owner && (hasUpdate || !supportsUpdate) && <div className="node-actions">
          ...
          <p id="update-state" className="meta">{supportsUpdate ? `Update ${device.agent_version} → ${relayVersion}. Stops active processes while restarting.` : "This node predates remote updates."}</p>

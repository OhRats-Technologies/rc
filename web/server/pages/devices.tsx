    ...
    <div className="page" data-device-page={device.id}>
      ...
      <section className="device-summary">
        ...
        <div className="node-actions">
          {owner && supportsUpdate && <button id="update-node" className="text-action" type="button" disabled={!device.online}>UPDATE NODE</button>}
          <p id="update-state" className="meta">{supportsUpdate ? owner ? "Stops active processes while restarting." : "Owners manage Relay Node updates." : owner ? "This node predates remote updates." : "Ask an owner to update this Relay Node."}</p>
      ...
      {owner && <section className="content-section danger-section"><div className="section-heading"><div><SectionBadge index="03">Device</SectionBadge><h2>Settings</h2></div></div><form method="post" action={`/devices/${device.id}/rename`} className="inline-form compact-inline-form"><label>Name<input name="name" defaultValue={device.name} required/></label><button className="or-button" type="submit">RENAME</button></form><a className="text-action danger-text" href={`/devices/${device.id}/delete`}>REMOVE DEVICE</a></section>}
    </div> });
}
